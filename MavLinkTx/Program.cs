...
class Program
{
    ...
    {
        Console.WriteLine("MavLinkTx started. Generating and sending random MavLink messages...");
        ...
        if (!messageIds.Any())
        {
            Console.WriteLine("No MavLink messages found in common.xml. Exiting.");
            return;
        }

        var random = new Random();
        byte packetSequence = 0; // MAVLink packet sequence number
        ...
        using (var udpClient = new UdpClient())
        {
            ...
            while (true)
            {
                ...
                {
                    Console.WriteLine($"Could not find message definition for ID: {randomMessageId}");
                }
